...
public class Nmap {
	...
	public RemoteHostList scanNmap(String rootPass, String rhost)
			throws Exception {
		// add command
		...
		commands.add("-c");
		if (rhost.contains("/")) {
			commands.add("echo " + rootPass + " | sudo -S nmap -sS -O -iL "
					+ rhost);
		} else {
				...
				if (stdoutLines[i].contains("PORT")) {
					...
					while (true) {
						if (stdoutLines[i + j].contains("MAC Address:")) {
							...
							String[] portProperties = stdoutLines[i + j].trim()
									.replaceAll(" +", " ").split(" ");
							Ports.add(new Port(portProperties[0],
									portProperties[1], portProperties[2]));
						}
						j++;
					}
					break;
