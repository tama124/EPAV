...
public class FormFeatureOne extends JFrame {
	...
	public FormFeatureOne() {
		...
		radiobtnOptional = new JRadioButton(
				"Select some vulnerabilities in list below");
		...
		checkTreeManager = new CheckTreeManager(treeVulSelection = new JTree(
				this.getVulNameListtoTreeView()));
		...
		btnRun.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				final String currentTime = new SimpleDateFormat(
				...
				if (util.checkValidIpAddress(lhost)
						&& (util.checkValidIpAddress(rhost) || rhost
								.contains("/"))) {
					if (util.checkValidRootPass(rootPass)) {
						try {
							...
							if (radioButtonFlag == true) {
								...
								TreePath checkedPaths[] = checkTreeManager
										.getSelectionModel()
										.getSelectionPaths();
								for (int i = 0; i < checkedPaths.length; i++) {
									listOfCheckedVulName.add(checkedPaths[i]
							...
							if (listOfCheckedVulName.size() == 0) {
								...
								updateProgressBarSmoothly(
										"Scanning your remote hosts",
										MIN_PERCENT_VALUE);
								execute(lhost, rhost, rootPass,
										listOfCheckedVulName, currentTime);
							}
						} catch (Exception e) {
							showMessageErrorDialog("You must check at least one vulnerability!");
						}
			...
			public void run() {
				try {
					...
					for (File file : listOfVulScripts) {
						if (file.isFile()
								&& file.getName().toLowerCase().endsWith(".sh")) {
	...
	private void execute(final String lhost, final String rhost,
			final String rootPass,
			final ArrayList<String> listOfCheckedVulName,
		...
		Thread t = new Thread(new Runnable() {
			public void run() {
				try {
					...
					final RemoteHostList remoteHostList = new Nmap().scanNmap(
							rootPass, rhost);
					if (remoteHostList == null) {
						showMessageErrorDialog("Something was wrong! Please try again!");
					} else {
						for (RemoteHost h : remoteHostList
								.getListOfRemoteHost()) {
							...
							int totalProcess = (remoteHostList
									.getListOfRemoteHost().size() * (listFilter
									.size() + 1)); // +1 process nmap
							eachProcessPercentValue = MAX_PERCENT_VALUE
									/ totalProcess;
							tmpPercentValue = MAX_PERCENT_VALUE
									- (eachProcessPercentValue * totalProcess);
							updateProgressBarSmoothly("Exploiting "
									+ h.getIP()
									+ ": "
									+ listFilter.get(0).substring(0, 8)
											.replace(".sh", ""),
									eachProcessPercentValue);

							for (int i = 0; i < listFilter.size(); i++) {
								h.getVulnerabilities().add(
										new Vulnerability(listFilter
												.get(i)
												.substring(
														0,
														listFilter.get(i)
								...
								String metasploitResult = new Metasploit()
										.runMetasploit(lhost, h.getIP(),
												rootPass, listFilter.get(i));
								...
								if (metasploitResult.contains("was detected")) {
									h.getVulnerabilities()
											.getListOfVulnerability().get(i)
											.setState("Detected");
									for (int j = 0; j < listOfAllPatchName
											.size(); j++) {
										if (h.getVulnerabilities()
												.getListOfVulnerability()
												.get(i).getName().toLowerCase()
												.substring(0, 8)
												.equals("ms09_050")) {
											patchList
													.add(new Patch(
															"MS09_050_WindowsEnable.msi"));
											patchList
													.add(new Patch(
															"MS09_050_WindowsDisable.msi"));
											break;
										}
										if (listOfAllPatchName
												.get(j)
												.toLowerCase()
												.contains(
														h.getVulnerabilities()
											...
											patchList.add(new Patch(
													listOfAllPatchName.get(j)));
										}
									}
									h.getVulnerabilities()
											.getListOfVulnerability().get(i)
											.setPatchList(patchList);
									ioStream.copyPatchesToHosts(h.getIP(),
											patchList, currentTime);
								} else {
									h.getVulnerabilities()
											.getListOfVulnerability().get(i)
											.setState("Fixed");
	...
	private void updateProgressBarSmoothly(String currentProgressBarState,
			int newPercentValue) {
		if (newPercentValue == MIN_PERCENT_VALUE
				&& !currentProgressBarState.equals("Finishing")) {
			if (currentProgressBarState == "") {
				...
				progressBar.setValue(newPercentValue);
				progressBar.setString("Running " + newPercentValue + " % - "
						+ currentProgressBarState);
				currentPercentValue = 0;
			}
		} else {
			for (int i = currentPercentValue + 1; i <= currentPercentValue
					+ newPercentValue; i++) {
				progressBar.setValue(i);
				progressBar.setString("Running " + i + " % - "
						+ oldProgressBarState);
				try {
					Thread.sleep(30);
				} catch (Exception e) {
			...
			progressBar.setString("Running " + currentPercentValue + " % - "
					+ currentProgressBarState);
