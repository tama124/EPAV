...
public class FormResult extends JFrame {
    ...
    final private String columnNamesTablePortsList[] = { "Name", "State", "Service", "Warning", "Solution" };
    
    private JTable[] tableVulsList;
    private String[][] rowVulDatas;
    final private String columnNamesTableVulsList[] = { "Name", "State", "Patches" };
    ...
    public FormResult(final RemoteHostList remoteHostList, final String currentTime) {
        ...
        for (int i = 0; i < this.numberOfHosts; i++) {
        	...
        	hostIP = new JLabel("IP address: " + this.remoteHostList.getListOfRemoteHost().get(i).getIP());
        	hostMAC = new JLabel("MAC address: " + this.remoteHostList.getListOfRemoteHost().get(i).getMAC());
        	hostOS = new JLabel("Operating system details: " + this.remoteHostList.getListOfRemoteHost().get(i).getOS());
        	
        	//get remote host ports list
        	hostPortsList = new JLabel("Ports:");
        	int numberOfPorts = this.remoteHostList.getListOfRemoteHost().get(i).getPorts().getListOfPort().size();
            rowPortDatas = new String[numberOfPorts][5];
        	for (int j = 0; j < numberOfPorts; j++) {
				rowPortDatas[j][0] = remoteHostList.getListOfRemoteHost().get(i).getPorts().getListOfPort().get(j).getName();
				rowPortDatas[j][1] = remoteHostList.getListOfRemoteHost().get(i).getPorts().getListOfPort().get(j).getState();
				rowPortDatas[j][2] = remoteHostList.getListOfRemoteHost().get(i).getPorts().getListOfPort().get(j).getService();
				rowPortDatas[j][3] = remoteHostList.getListOfRemoteHost().get(i).getPorts().getListOfPort().get(j).getWarning();
				rowPortDatas[j][4] = remoteHostList.getListOfRemoteHost().get(i).getPorts().getListOfPort().get(j).getSolution();
			}
			tablePortsList[i] = new JTable(rowPortDatas, columnNamesTablePortsList);
			JTableHeader tableHeaderPortsList = tablePortsList[i].getTableHeader(); 
        	
        	// get remote host vulnerabilities list
        	hostVulsList = new JLabel("Scanned vulnerabilities:");
        	int numberOfVuls = this.remoteHostList.getListOfRemoteHost().get(i).getVulnerabilities().getListOfVulnerability().size();
            rowVulDatas = new String[numberOfVuls][3];
            for (int j = 0; j < numberOfVuls; j++) {
				rowVulDatas[j][0] = remoteHostList.getListOfRemoteHost().get(i).getVulnerabilities().getListOfVulnerability().get(i).getName();
				rowVulDatas[j][1] = remoteHostList.getListOfRemoteHost().get(i).getVulnerabilities().getListOfVulnerability().get(i).getState();
				rowVulDatas[j][2] = remoteHostList.getListOfRemoteHost().get(i).getVulnerabilities().getListOfVulnerability().get(i).getPatchList().display();
			}
			tableVulsList[i] = new JTable(rowVulDatas, columnNamesTableVulsList);
			JTableHeader tableHeaderVulsList = tableVulsList[i].getTableHeader();
	        ...
	        tabbedPane.add(new JScrollPane(containerHostDetailsList[i], JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER), this.remoteHostList.getListOfRemoteHost().get(i).getIP());
	        ...
	        btnExportCsv.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					try {
						new IOStream().createCSVResultFile(remoteHostList, currentTime);
						JOptionPane.showMessageDialog(null, "Exporting successfully!", "Informing", JOptionPane.INFORMATION_MESSAGE);
					} catch (Exception e) {
